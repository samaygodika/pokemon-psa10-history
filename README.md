...
| `listings_checked_at` | UTC time of the last **believable** answer from alt.xyz about what is for sale right now at PSA 10 (from 2026-09-25: the nightly scope daily, everything else on the weekly full run). **Blank = never checked**, and then every live column below is blank too (unknown, not "nothing listed"). alt.xyz's live-listings query fails silently — for hours at a time it answers "nothing" for every card (2026-09-25: from 14:20 UTC to the end of the run, and at 20:15 UTC for an auction eBay showed live with 53 bids). The scraper only records an empty answer as a check while at least 10% of the last 200 cards got a listing (`ListingsHealth` in `alt_scraper.py`); otherwise the card keeps its previous snapshot and this time stays at that earlier check, so it can be days old — show it. |
...
| `lowest_bin_price`, `lowest_bin_source`, `lowest_bin_url` | the cheapest PSA 10 Buy It Now listing at that check (PokeSniper's `lowestListingPrice`). It can sell or be pulled between checks, and alt.xyz itself keeps ended BINs in its feed for months (one returned as live on 2026-09-25 had ended on June 16), so treat the link as "was listed at", not "is listed at". A BIN alt.xyz has not shown again for 7 days is dropped (`history/ingest.py`). |
