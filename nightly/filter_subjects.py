...
usage: filter_subjects.py <all_pokemon_cards.json> <subjects.txt> <out_list.txt> [limit]
"""
import json, re, sys
...
subjects = [l.strip() for l in subjects_path.read_text().splitlines() if l.strip() and not l.startswith("#")]
...
with open(out_path, "w") as f:
    f.write(f"# {len(docs)} of {len(index)} indexed cards whose subject matches one of {len(subjects)} names in {subjects_path.name}\n")
...
print(f"{len(docs)} of {len(index)} cards match {len(subjects)} subjects -> {out_path}" + (f"  (limit {limit})" if limit else ""))
if zero:
